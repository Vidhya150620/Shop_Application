...
router.post('/generateReport',auth.authenticateToken,(req,res)=>{
    ...
    connection.query(query,[orderDetails.name,generatedUuid,orderDetails.email,orderDetails.contactNumber,orderDetails.paymentMethod,orderDetails.totalAmount,orderDetails.productDetails,res.locals.email],(err,results)=>{
        if(!err){
            ejs.renderFile(path.join(__dirname,'',"report.ejs"),{productDetails:productDetailsReport,name:orderDetails.name,email:orderDetails.email,
                contactNumber:orderDetails.contactNumber,paymentMethod:orderDetails.paymentMethod,totalAmount:orderDetails.totalAmount},(err,results)=>{
                    if(err){
...
router.get('/getBills',auth.authenticateToken,(req,res)=>{
    var query = "select * from bill order by id DISC";
...
router.delete('/delete/:id',auth.authenticateToken,(req,res,next)=>{
    ...
    connection.query(query,[id],(err,results)=>{
        if(!err){
            if(results.length ==0){
                return res.status(404).json({ message:"Bill id not exists"})
            }
